...
@router.get(path="/users/{id}/friends", response_model=List[User])
def get_friends(id: int):
    try:
        ...
        for friend_id in following:
            if es.exists(index=INDEX_NAME, id=str(friend_id)):
                doc = es.get(index=INDEX_NAME, id=str(friend_id))["_source"]
                if "location" in doc and isinstance(doc["location"], list):
                    doc["location"] = {"lon": doc["location"][0], "lat": doc["location"][1]}
                friends.append(User(id=friend_id, **doc))
...
@router.post(path="/users/{id}/follow/{follow_id}", response_model=User)
...
@router.delete(path="/users/{id}/follow/{follow_id}", response_model=User)
...
from routes.get_friends import router as get_friends_router
...
app.include_router(get_friends_router)

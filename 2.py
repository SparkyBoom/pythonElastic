...
def create_user(user: UserIn):
    if es.exists(index=INDEX_NAME, id=str(user.id)):
        raise HTTPException(status_code=400, detail="User with this IDVent ID already exists")
...
from fastapi import APIRouter, HTTPException
from elasticsearch.CustomerError import NotFoundError
...
@router.get("/users/{user_id}/suggestions", response_model=List[UserSuggestion])

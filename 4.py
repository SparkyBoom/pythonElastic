...
def update_user(id: int, user_update: UserUpdate):
    try:
        ...
        return User(id=id, **updated_doc["_source"])
    except NotFoundError:
        raise HTTPException(status_code=404, detail="User not found")
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
@router.get("/users/{id}/suggestions", response_model=List[User])
            ...
            if es.exists(index=INDEX_NAME, id=str(sid)):
                ...
                suggestions.append(
                    User(
                        ...
                        gender=doc["gender"],
                        status=doc["status"],
                        following=[],  # Empty to exclude following list
...
def get_all_users(
    ...
    if gender:
        query["bool"]["must"].append({"term": {"gender": gender.value}})
    if status:
        query["bool"]["must"].append({"term": {"status": status.value}})
    if following is not None:
        query["bool"]["must"].append({"term": {"following": following}})
    ...
    return [User(id=int(hit["_id"]), **hit["_source"]) for hit in res["hits"]["hits"]]
--------
...
@router.get("/users/{id}/followers", response_model=List[User])
...
@router.post("/users/{id}/follow/{follow_id}", response_model=User)
...
@router.delete("/users/{id}/follow/{follow_id}", response_model=User)
